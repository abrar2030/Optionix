...
import { ActivityIndicator, Card, Title, Paragraph, List, Divider, Text as PaperText, useTheme, Button, TextInput } from 'react-native-paper';

const WatchlistScreen = () => {
  const [watchlist, setWatchlist] = useState([
    { symbol: 'MSFT', price: 420.55, change: '+1.1%' },
    { symbol: 'NVDA', price: 950.02, change: '-0.5%' },
    { symbol: 'AMZN', price: 180.30, change: '+0.8%' },
  ]);
  ...
  const renderWatchlistItem = ({ item }) => (
    <List.Item
      ...
      left={() => <Button icon="delete" onPress={() => removeSymbol(item.symbol)} compact>Remove</Button>}
    />
  );
        ...
        <Card.Content>
          {loading ? (
            ...
            <FlatList
              ...
              ListEmptyComponent={<Paragraph style={styles.emptyListText}>Your watchlist is empty. Add symbols above.</Paragraph>}
            />
          )}
        </Card.Content>
      </Card>
