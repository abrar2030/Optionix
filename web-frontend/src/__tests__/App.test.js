...
jest.mock('react-chartjs-2', () => ({
  ...
  Doughnut: () => <div data-testid="mocked-doughnut-chart">Mocked Doughnut Chart</div>,
...
jest.mock("../utils/api", () => { console.log("[DEBUG] jest.mock factory for ../utils/api CALLED"); return { __esModule: true, default: mockApi }; });

// Mock AppContext - specifically the useAppContext hook
let mockUserValue = null;
let mockLoadingValue = false;
const mockSetUser = jest.fn(newUser => { mockUserValue = newUser; });
const mockSetLoading = jest.fn(newLoading => { mockLoadingValue = newLoading; });
...
jest.mock('../utils/AppContext', () => {
  ...
  return {
    ...
    useAppContext: () => ({ // Override only useAppContext
      user: mockUserValue,
      setUser: mockSetUser,
...
describe('App Component', () => {
  beforeEach(() => {
    ...
    mockSetLoading.mockImplementation(newLoading => { mockLoadingValue = newLoading; });
  ...
  test('renders login page by default', async () => {
    renderApp();
    await waitFor(() => expect(screen.getByRole("heading", { name: /login/i, level: 2 })).toBeInTheDocument());
  });
  ...
  test('handles successful login', async () => {
    ...
    await waitFor(() => {
        expect(mockSetUser).toHaveBeenCalledWith(expect.objectContaining({ username: 'testuser' }));
    });
  ...
  test('navigates to registration page', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('heading', { name: /create account/i, level: 2 })).toBeInTheDocument());
  });
