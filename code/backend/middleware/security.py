...
class AdvancedRateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp):
        ...
        self.endpoint_limits = {
            ...
            "/predict_volatility": {"requests": 10, "window": 60},  # 10 predictions per minute
        }
    ...
    async def _check_rate_limits(
        ...
        rate_key = f"rate_limit:{identifier}:{endpoint}:{current_time // window_seconds}"
...
class AuditLoggingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp):
        ...
        self.sensitive_endpoints = [
            "/auth/login", "/auth/register", "/trades", "/positions",
            "/accounts", "/predict_volatility"
        ]
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self._should_log_request(request):
            await self._log_request_response(request, response, request_data, process_time)
