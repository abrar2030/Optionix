...
class User(Base):
    ...
    role = Column(String(50), default="trader")  # admin, trader, viewer, compliance_officer, risk_manager, api_user
    ...
    # KYC and compliance
    kyc_status = Column(String(50), default="pending")  # pending, approved, rejected, under_review
...
class Account(Base):
    ...
    account_type = Column(String(50), default="standard")  # standard, premium, institutional, demo
    account_status = Column(String(20), default="active")  # active, suspended, closed, frozen
...
class Trade(Base):
    ...
    # Execution details
    status = Column(String(20), default="pending")  # pending, executed, cancelled, failed, rejected
    ...
    transaction_monitoring = relationship("TransactionMonitoring", back_populates="trade")
...
class AuditLog(Base):
    ...
    action_category = Column(String(50), nullable=False)  # authentication, trading, compliance, admin
...
class KYCDocument(Base):
    ...
    # Document details
    document_type = Column(String(50), nullable=False)  # passport, national_id, drivers_license
...
class SanctionsCheck(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    check_id = Column(String(36), unique=True, index=True, default=lambda: str(uuid.uuid4()))
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    ...
    # Resolution
    resolution_status = Column(String(20), default="pending")  # pending, cleared, escalated
    resolution_notes = Column(Text, nullable=True)
    resolved_by = Column(Integer, ForeignKey("users.id"), nullable=True)
    resolved_at = Column(DateTime, nullable=True)
    
    # Relationships
    user = relationship("User", back_populates="sanctions_checks", foreign_keys=[user_id])
...
class TransactionMonitoring(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    alert_id = Column(String(36), unique=True, index=True, default=lambda: str(uuid.uuid4()))
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    trade_id = Column(Integer, ForeignKey("trades.id"), nullable=True)
    
    # Alert details
    alert_type = Column(String(100), nullable=False)
    alert_category = Column(String(50), nullable=False)  # threshold, pattern, velocity, behavioral
    ...
    investigation_priority = Column(String(20), default="medium")  # low, medium, high, urgent
