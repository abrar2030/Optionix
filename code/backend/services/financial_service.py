...
class FinancialCalculationService:
    ...
    def calculate_option_greeks(
        ...
        try:
            ...
            # Rho
            if is_call:
                rho = K * T * math.exp(-r * T) * N_d2 / 100  # Per 1% change in interest rate
            else:
    ...
    def calculate_var(
        ...
        try:
            ...
            daily_volatility = volatility / Decimal(str(math.sqrt(self.trading_days_per_year)))
            
            # Scale for time horizon
            horizon_volatility = daily_volatility * Decimal(str(math.sqrt(time_horizon)))
    ...
    def validate_position_limits(
        ...
        try:
            ...
            if position_value > Decimal(str(settings.max_position_size)):
                violations.append(f"Position value {position_value} exceeds maximum {settings.max_position_size}")
            
            # Check minimum position size
            if position_value < Decimal(str(settings.min_position_size)):
                violations.append(f"Position value {position_value} below minimum {settings.min_position_size}")
