...
class ReconciliationRecord(Base):
    ...
    # Status
    reconciliation_status = Column(String(20), nullable=False)  # matched, unmatched, investigating
    tolerance_threshold = Column(Numeric(precision=18, scale=8), default=Decimal('0.01'))
...
class RiskMetric(Base):
    ...
    confidence_level = Column(Numeric(precision=5, scale=4), nullable=True)  # e.g., 0.95 for 95%
...
class FinancialStandardsService:
    ...
    def create_financial_audit_log(
        ...
        try:
            ...
            audit_log = FinancialAuditLog(
                ...
                amount=amount,
                currency="USD",
                previous_state=json.dumps(previous_state, default=str) if previous_state else None,
    ...
    def perform_data_integrity_check(
        ...
        try:
            check_id = f"DIC_{int(datetime.utcnow().timestamp())}_{entity_type}_{entity_id}"
            ...
            if variance:
                discrepancy_amount = abs(Decimal(str(variance.get('total_difference', 0))))
                ...
                try:
                    ...
                    if difference != 0:
                        variance[key] = {
                            ...
                            "percentage": str((difference / expected_val * 100).quantize(Decimal('0.01')))
                        }
    ...
    def perform_reconciliation(
        ...
        try:
            reconciliation_id = f"REC_{reconciliation_type}_{int(datetime.utcnow().timestamp())}"
    ...
    def calculate_risk_metrics(
        ...
        try:
            ...
            for metric_type in metric_types:
                ...
                warning_threshold = limit_value * Decimal('0.8') if limit_value else None
    ...
    def _calculate_specific_risk_metric(
        ...
        if metric_type == RiskMetricType.VAR:
            ...
            return self._calculate_leverage_ratio(db, entity_type, entity_id, business_date)
        elif metric_type == RiskMetricType.LIQUIDITY_RATIO:
            return self._calculate_liquidity_ratio(db, entity_type, entity_id, business_date)
        else:
            # Default calculation
            return Decimal('0')
        ...
        try:
            ...
            if entity_type == "account":
                positions = db.query(Position).filter(
                    Position.account_id == int(entity_id),
    ...
    def check_sox_compliance(
        ...
        try:
            ...
            required_roles = self.sox_controls["authorization_levels"].get(transaction_type, [])
            ...
            if amount > Decimal('100000'):  # Large transaction threshold
                compliance_results["controls_checked"].append("large_transaction_approval")
    ...
    def perform_daily_reconciliation(self, db: Session, business_date: datetime) -> List[ReconciliationRecord]:
        ...
        try:
            ...
            for account in accounts:
                ...
                expected_balance = self._calculate_expected_balance(db, account.id, business_date)
