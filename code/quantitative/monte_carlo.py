...
class EnhancedMonteCarloSimulator:
    ...
    def geometric_brownian_motion(
        ...
        if self.params.variance_reduction == VarianceReduction.ANTITHETIC:
            ...
            random_normals = self._generate_sobol_normals(
                self.params.time_steps, n_sims
            )
        else:
        ...
        for t in range(1, self.params.time_steps + 1):
            ...
            diffusion_term = (
                self.params.volatility * np.sqrt(dt) * random_normals[t - 1]
            )
    ...
    def heston_model(
        ...
        for t in range(1, self.params.time_steps + 1):
            ...
            variance_diffusion = (
                sigma_v * np.sqrt(np.maximum(variance_paths[t - 1], 0) * dt) * z2[t - 1]
            )
            ...
            price_diffusion = (
                np.sqrt(np.maximum(variance_paths[t - 1], 0) * dt) * z1[t - 1]
            )
            price_paths[t] = price_paths[t - 1] * np.exp(
                price_drift - 0.5 * variance_paths[t - 1] * dt + price_diffusion
            )

        return price_paths, variance_paths

    def _generate_sobol_normals(
        self, time_steps: int, n_simulations: int
    ) -> np.ndarray:
    ...
    def calculate_payoff(
        self, paths: np.ndarray, payoff_spec: OptionPayoff, discount_factor: float = 1.0
    ) -> np.ndarray:
        ...
        if payoff_spec.option_type == "call":
            ...
            if payoff_spec.barrier_type == "up_and_out":
                payoffs = np.where(
                    barrier_breached,
                    0,
                    np.maximum(final_prices - payoff_spec.strike_price, 0),
                )
    ...
    def price_option(
        self, payoff_spec: OptionPayoff, risk_free_rate: float = 0.0
    ) -> SimulationResult:
        ...
        try:
            ...
            return SimulationResult(
                ...
                paths=(
                    paths if self.params.num_simulations <= 1000 else None
                ),  # Store paths only for small simulations
                payoffs=payoffs,
                greeks=greeks,
    ...
    def _calculate_greeks(
        self, payoff_spec: OptionPayoff, risk_free_rate: float
    ) -> Dict[str, float]:
        """Calculate Greeks using finite differences"""
        try:
            ...
            params_up = SimulationParameters(
                ...
                num_simulations=min(
                    self.params.num_simulations, 10000
                ),  # Reduce for Greeks calculation
            ...
            payoffs_up = simulator_up.calculate_payoff(
                paths_up,
                payoff_spec,
                np.exp(-risk_free_rate * self.params.time_horizon),
            )
            ...
            payoffs_down = simulator_down.calculate_payoff(
                paths_down,
                payoff_spec,
                np.exp(-risk_free_rate * self.params.time_horizon),
            )
            ...
            payoffs_vol_up = simulator_vol_up.calculate_payoff(
                paths_vol_up,
                payoff_spec,
                np.exp(-risk_free_rate * self.params.time_horizon),
            )
            ...
            payoffs_base = self.calculate_payoff(
                paths_base,
                payoff_spec,
                np.exp(-risk_free_rate * self.params.time_horizon),
            )
...
class MonteCarloSimulator:
    ...
    def asian_option_price(
        ...
        payoff_spec = OptionPayoff(option_type="asian_call", strike_price=K)
