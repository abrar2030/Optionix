...
class CalibrationEngine:
        ...
        if isinstance(option_data, dict):
            ...
            if "calls" in option_data:
                for call in option_data["calls"]:
                    options.append({
                        "strike": call["strike"],
                        "expiry": call.get("expiry", 0.5),  # Default to 6 months if not specified
                        "price": call["price"] if "price" in call else self._bs_price_from_iv(
            ...
            if "puts" in option_data:
                for put in option_data["puts"]:
                    options.append({
                        "strike": put["strike"],
                        "expiry": put.get("expiry", 0.5),  # Default to 6 months if not specified
                        "price": put["price"] if "price" in put else self._bs_price_from_iv(
            ...
            if "calls" in option_data:
                for call in option_data["calls"]:
                    options.append({
                        ...
                        "time_to_expiry": call.get("expiry", 0.5),  # Default to 6 months if not specified
                        "market_vol": call["iv"]
                    })
            if "puts" in option_data:
                for put in option_data["puts"]:
                    options.append({
                        ...
                        "time_to_expiry": put.get("expiry", 0.5),  # Default to 6 months if not specified
                        "market_vol": put["iv"]
                    })
        else:
            # Assume list format
    ...
    def calibrate_local_volatility(self, option_data, spot, rate, dividend=0):
            ...
            if "calls" in option_data:
                for call in option_data["calls"]:
                    options.append({
                        ...
                        "expiry": call.get("expiry", 0.5),  # Default to 6 months if not specified
                        "price": call["price"] if "price" in call else self._bs_price_from_iv(
                            call["iv"], call["strike"], 0.5, "call", spot, rate, dividend
                        ),
                        "option_type": "call"
                    })
            if "puts" in option_data:
                for put in option_data["puts"]:
                    options.append({
                        ...
                        "expiry": put.get("expiry", 0.5),  # Default to 6 months if not specified
                        "price": put["price"] if "price" in put else self._bs_price_from_iv(
                            put["iv"], put["strike"], 0.5, "put", spot, rate, dividend
                        ),
                        "option_type": "put"
                    })
        else:
        ...
        # Calibrate model
        local_vol_surface, strike_grid, time_grid = dupire.calibrate(options, spot, rate, dividend)
        
        return local_vol_surface, strike_grid, time_grid
    
    def _bs_price_from_iv(self, iv, strike, time_to_expiry, option_type, spot=450.0, rate=0.02, dividend=0.01):
        """
        ...
        # Calculate d1 and d2
        d1 = (np.log(spot / strike) + (rate - dividend + 0.5 * iv**2) * time_to_expiry) / (iv * np.sqrt(time_to_expiry))
