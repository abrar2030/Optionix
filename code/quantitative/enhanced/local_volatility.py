...
class DupireLocalVolModel:
        ...
        for i, t in enumerate(time_grid):
            for j, k in enumerate(strike_grid):
                ...
                matching_options = [
                    ...
                    if opt["strike"] == k and (
                        (isinstance(opt["expiry"], str) and 
                         (datetime.strptime(opt["expiry"], "%Y-%m-%d") - datetime.now()).days / 365.0 == t)
                        or opt["expiry"] == t
                    )
                ]
    ...
    def _calculate_implied_volatility(self, price, spot, strike, time_to_expiry, rate, dividend, option_type):
        """
        ...
        def objective(sigma):
            ...
            d1 = (np.log(spot / strike) + (rate - dividend + 0.5 * sigma**2) * time_to_expiry) / (sigma * np.sqrt(time_to_expiry))
            d2 = d1 - sigma * np.sqrt(time_to_expiry)
            
            if option_type.lower() == "call":
                bs_price = spot * np.exp(-dividend * time_to_expiry) * stats.norm.cdf(d1) - strike * np.exp(-rate * time_to_expiry) * stats.norm.cdf(d2)
            else:  # put
                bs_price = strike * np.exp(-rate * time_to_expiry) * stats.norm.cdf(-d2) - spot * np.exp(-dividend * time_to_expiry) * stats.norm.cdf(-d1)
    ...
    def local_volatility(self, spot, strike, time_to_expiry):
        ...
        if self.local_vol_surface is None:
            ...
            logger.warning("Local volatility surface not calibrated, using default value")
    ...
    def price_option(self, spot, strike, time_to_expiry, rate, dividend, option_type="call", num_paths=10000, num_steps=100):
        """
        ...
        for i in range(num_steps):
            ...
            local_vols = np.array([self.local_volatility(spot, paths[j, i], t) for j in range(num_paths)])
            
            # Generate random numbers
            z = np.random.normal(0, 1, num_paths)
            
            # Update paths
            paths[:, i+1] = paths[:, i] * np.exp((rate - dividend - 0.5 * local_vols**2) * dt + local_vols * np.sqrt(dt) * z)
...
class CEVModel:
    ...
    def price_option(self, spot, strike, time_to_expiry, rate, dividend, option_type="call", method="monte_carlo", num_paths=10000, num_steps=100):
        ...
        if method == "monte_carlo":
            ...
            return self._price_analytical(spot, strike, time_to_expiry, rate, dividend, option_type)
        else:
    ...
    def _price_monte_carlo(self, spot, strike, time_to_expiry, rate, dividend, option_type, num_paths, num_steps):
        ...
        for i in range(num_steps):
            ...
            # Update paths
            paths[:, i+1] = paths[:, i] * np.exp((rate - dividend - 0.5 * local_vols**2 * paths[:, i]**(2*(beta-1))) * dt + local_vols * paths[:, i]**(beta-1) * np.sqrt(dt) * z)
    ...
    def _price_analytical(self, spot, strike, time_to_expiry, rate, dividend, option_type):
        """
        ...
        if abs(beta - 1) < 1e-6:
            ...
            if option_type.lower() == "call":
                price = spot * np.exp(-dividend * time_to_expiry) * stats.norm.cdf(d1) - strike * np.exp(-rate * time_to_expiry) * stats.norm.cdf(d2)
            else:  # put
                price = strike * np.exp(-rate * time_to_expiry) * stats.norm.cdf(-d2) - spot * np.exp(-dividend * time_to_expiry) * stats.norm.cdf(-d1)
        ...
        d1 = (np.log(spot / strike) + (rate - dividend + 0.5 * effective_vol**2) * time_to_expiry) / (effective_vol * np.sqrt(time_to_expiry))
